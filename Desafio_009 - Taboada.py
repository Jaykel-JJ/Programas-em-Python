# -*- coding: utf-8 -*-
"""
Created on Mon Oct 24 09:21:50 2022

@author: jayke
"""

n = int(input('Digit eum valor para saber sua taboada: '))

#aqui começa o esquema de montar a taboada em cada print (jeito nao recomendado)
print('='*10)

print(n,' x 0 = {:2}'.format(n*0))
print(n,' x 1 = {:2}'.format(n*1))
print(n,' x 2 = {:2}'.format(n*2))
print(n,' x 3 = {:2}'.format(n*3))
print(n,' x 4 = {:2}'.format(n*4))
print(n,' x 5 = {:2}'.format(n*5))
print(n,' x 6 = {:2}'.format(n*6))
print(n,' x 7 = {:2}'.format(n*7))
print(n,' x 8 = {:2}'.format(n*8))
print(n,' x 9 = {:2}'.format(n*9))
print(n,' x 10 = {:2}'.format(n*10))

print('='*10)
